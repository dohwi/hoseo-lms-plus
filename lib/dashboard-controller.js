(function (global, factory) {
    ...
    function create(options) {
        ...
        function getRenderableActivities(data, week) {
            ...
            const coursesWithActivities = new Set(activities.map((item) => item.courseId));
            data.allCourseNames.forEach((course) => {
                if (week !== core.OTHER_WEEK_NUM && !coursesWithActivities.has(course.courseId)) {
                    activities.push({
                        ...
                        type: '-',
                        href: '',
                        nameHtml: '<span>등록된 학습 자료나 활동이 없습니다.</span>',
                        optionsHtml: '-',
                        statusText: '-',
                        isCompleted: true,
                        isNeutral: true
        ...
        function replacePageContent(forceRefresh) {
            runtime.resetRequestQueue();
            savedCourseIds = core.getCourseIds(doc);
