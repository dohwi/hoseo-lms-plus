(function (global, factory) {
    ...
    function create(options) {
        ...
        function getCourseContext() {
            if (!savedCourseInfo) savedCourseInfo = core.getCourseInfoList(doc);
            const userContext = core.getUserContext(doc);
            const courseIds = savedCourseInfo.map((info) => info.id);
            return {
                courseIds: courseIds,
                courseInfoList: savedCourseInfo.slice(),
                userContext: userContext,
                cacheKey: core.buildCacheKey(userContext, courseIds)
            };
        }
        ...
        function renderDashboardData(data) {
            ...
            const hasOtherWeekLearning = (itemsByWeek.get(core.OTHER_WEEK_NUM) || []).length > 0 ||
                (assignsByWeek.get(core.OTHER_WEEK_NUM) || []).length > 0 ||
                (activitiesByWeek.get(core.OTHER_WEEK_NUM) || []).some(function (item) { return item.type !== '공지사항'; });
            if (!hasOtherWeekLearning && weekSet.size > 1) weekSet.delete(core.OTHER_WEEK_NUM);
            ...
            const unassignedNotices = core.dedupActivities(
                data.allActivities.filter(function (item) {
                    return item.type === '공지사항' && item.weekNum === core.OTHER_WEEK_NUM;
                })
            );
            const renderState = {
            ...
            const initialWeekIdx = renderState.weekIdx;

            renderState.render = function () {
                const week = renderState.sortedWeeks[renderState.weekIdx];
                const weekActivities = getRenderableActivities(data, week);
                const activities = core.dedupActivities(
                    weekActivities.concat(renderState.weekIdx === initialWeekIdx && week !== core.OTHER_WEEK_NUM ? unassignedNotices : [])
                );
        ...
        async function fetchAndRender(courseInfoList, forceRefresh) {
            ...
            if (!data) {
                try {
                    data = await service.fetchAllCourseData(courseInfoList, function (loaded, total) {
        ...
        function replacePageContent(forceRefresh) {
            if (currentMount && currentHost) {
                ui.restoreHost(currentHost, currentMount);
                currentMount = null;
                currentHost = null;
            }
            runtime.resetRequestQueue();
            const latestInfo = core.getCourseInfoList(doc);
            if (latestInfo.length) savedCourseInfo = latestInfo;
            ...
            fetchAndRender(getCourseContext().courseInfoList, forceRefresh);
        }
