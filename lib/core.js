(function (global, factory) {
    ...
    const CACHE_MAX_PAYLOAD_BYTES = 4 * 1024 * 1024;
    ...
    const SELECTORS = {
        ...
        attendanceTable: ['.local-ubonattend table.table-coursemos', 'table.table-coursemos', 'table.table-striped', 'table.table'],
        courseSection: ['li.section.main', 'li.section', 'ul.weeks > li.section'],
        activityTable: ['.generaltable', 'table.table', 'table'],
        quizTable: ['.generaltable', 'table.table', 'table'],
        irregularCourseTypes: ['CMS_ON', 'IR', 'CMS_E', 'CMS_O', 'CMS_M']
    };
    ...
    function isSafeUrl(url, baseUrl) {
        if (!url) return false;
        try {
            const expectedBase = new URL(baseUrl || DEFAULT_BASE_URL);
            const parsed = new URL(url, expectedBase);
            return SAFE_PROTOCOLS.has(parsed.protocol) && parsed.origin === expectedBase.origin;
        } catch {
            return false;
        }
    }
    ...
    function queryWithFallback(doc, selectors) {
        const selectorList = Array.isArray(selectors) ? selectors : [selectors];
        for (let i = 0; i < selectorList.length; i += 1) {
            const match = (doc || document).querySelector(selectorList[i]);
    ...
    // HTML 문자열을 화이트리스트 기반으로 새니타이즈하여 DocumentFragment 반환
    // INLINE_TAGS(A,B,BR,EM,I,IMG,SMALL,SPAN,STRONG)만 요소로 유지
    // SCRIPT/STYLE/IFRAME 제거, onclick 등 이벤트 속성 제거, javascript: URL 차단
    ...
    // 추출 순서: activityKey 필드 → URL 식별자(href/materialHref/viewUrl) → 정규화된 이름(nameHtml/materialHtml/titleHtml/nameText)
    // URL 식별자는 pathname + '?' + 첫 매칭 query param (cmid/coursemodule/activity/id) 순서로 추출
    // 추출 실패 시 null 반환 (빈 문자열 아님 — 호출자에서 null 가드 필요)
    function buildActivityKey(item, baseUrl) {
        if (!item) return null;
        if (item.activityKey) return item.activityKey;
        const url = item.href || item.materialHref || item.viewUrl;
        if (url) {
            const identifier = getActivityIdentifier(url, baseUrl || DEFAULT_BASE_URL);
            if (identifier) return identifier;
        }
        const nameSource = item.nameHtml || item.materialHtml || item.titleHtml || item.nameText || '';
        const normalized = normalizeComparableText(nameSource);
    ...
    function dedupActivities(arr) {
        ...
        return arr.filter((item) => {
            const activityKey = buildActivityKey(item);
            if (!activityKey) return false;
            const key = [item.courseId, item.weekNum, activityKey].join('|');
    ...
    // "[MM.DD~MM.DD]" 형태의 기간 문자열에서 시작/종료 Date 추출
    // 연말(12월→1월)을 넘는 기간은 자동 보정
    ...
    // 현재 날짜 기준으로 표시할 주차의 인덱스를 결정
    // 우선순위: 현재 진행중인 주차 → 가장 최근에 끝난 주차 → 첫 번째 정규 주차
    ...
    function getCourseInfoList(doc) {
        const seen = new Set();
        const irregularSet = new Set(SELECTORS.irregularCourseTypes);
        return Array.from((doc || document).querySelectorAll(SELECTORS.courseItems))
            .map((element) => {
                ...
                return { id: id, courseType: courseType, isIrregular: irregularSet.has(courseType) };
            })
            .filter((item) => {
    ...
    function isValidCachePayload(payload) {
        return Boolean(payload) && typeof payload === 'object' && !Array.isArray(payload) &&
            Array.isArray(payload.allItems) && Array.isArray(payload.allAssigns) &&
            Array.isArray(payload.allActivities) && Array.isArray(payload.allCourseNames) &&
            Array.isArray(payload.warnings) && typeof payload.sessionExpired === 'boolean';
    }

    function isValidCacheEntry(entry) {
        return Boolean(entry) && typeof entry === 'object' && !Array.isArray(entry) &&
            Number.isFinite(entry.timestamp) && isValidCachePayload(entry.data);
    }
    ...
    function createCacheStore(storage) {
            ...
            try {
                ...
                for (const key of keys) {
                    try {
                        ...
                        if (!isValidCacheEntry(parsed) || utf8ByteLength(raw) > CACHE_MAX_PAYLOAD_BYTES || Date.now() - parsed.timestamp > CACHE_TTL) {
        ...
        function get(cacheKey) {
            ...
            try {
                ...
                if (!raw) return null;
                if (utf8ByteLength(raw) > CACHE_MAX_PAYLOAD_BYTES) {
                    target.removeItem(cacheKey);
                    return null;
                }
                const parsed = JSON.parse(raw);
                if (!isValidCacheEntry(parsed) || Date.now() - parsed.timestamp > CACHE_TTL) {
                    target.removeItem(cacheKey);
                    return null;
                }
                return parsed;
        ...
        function set(cacheKey, payload) {
            if (!target || !isValidCachePayload(payload)) return;
            try {
                const entryJson = JSON.stringify({ timestamp: Date.now(), data: payload });
                if (utf8ByteLength(entryJson) > CACHE_MAX_PAYLOAD_BYTES) return;
                pruneExpiredEntries();
                target.setItem(cacheKey, entryJson);
            } catch {
                return;
            }
        }
    ...
    function createAsyncCacheStore(extensionStorage, fallbackStorage) {
        ...
        async function pruneExpiredEntries() {
            if (!area || Date.now() - lastPrunedAt < CACHE_PRUNE_INTERVAL) return;
            lastPrunedAt = Date.now();
            try {
                const items = await getItems(area, null);
                if (!items) return;
                const expiredKeys = Object.keys(items).filter(function (key) {
                    if (!key.startsWith(CACHE_PREFIX)) return false;
                    const entry = items[key];
                    return !isValidCacheEntry(entry) || utf8ByteLength(JSON.stringify(entry)) > CACHE_MAX_PAYLOAD_BYTES || Date.now() - entry.timestamp > CACHE_TTL;
                });
        ...
        async function get(cacheKey) {
            ...
            try {
                ...
                const items = await getItems(area, cacheKey);
                const entry = items ? items[cacheKey] : null;
                if (!isValidCacheEntry(entry) || utf8ByteLength(JSON.stringify(entry)) > CACHE_MAX_PAYLOAD_BYTES) {
                    if (entry) await removeItems(area, [cacheKey]);
                    return null;
                }
        ...
        async function set(cacheKey, payload) {
            if (!isValidCachePayload(payload)) return;
            const entry = { timestamp: Date.now(), data: payload };
            if (utf8ByteLength(JSON.stringify(entry)) > CACHE_MAX_PAYLOAD_BYTES) return;
            if (!area) {
            ...
            try {
                await pruneExpiredEntries();
                await setItems(area, { [cacheKey]: entry });
            } catch {
                try {
                    const items = await getItems(area, null);
                    const evictionCandidates = Object.keys(items || {})
                        .filter(function (key) { return key.startsWith(CACHE_PREFIX) && key !== cacheKey; })
                        .sort(function (left, right) {
                            return ((items[left] && items[left].timestamp) || 0) - ((items[right] && items[right].timestamp) || 0);
                        });
                    if (!evictionCandidates.length) return;
                    const evictionCount = Math.max(1, Math.ceil(evictionCandidates.length / 2));
                    await removeItems(area, evictionCandidates.slice(0, evictionCount));
                    await setItems(area, { [cacheKey]: entry });
                } catch {
                    return;
                }
    ...
    function getManifestVersion() {
        const api = (typeof globalThis !== 'undefined') && (globalThis.browser || globalThis.chrome);
    ...
    return {
        CACHE_TTL: CACHE_TTL,
        CACHE_MAX_PAYLOAD_BYTES: CACHE_MAX_PAYLOAD_BYTES,
        DEFAULT_BASE_URL: DEFAULT_BASE_URL,
        LOGIN_PATTERN: LOGIN_PATTERN,
        OTHER_WEEK_NUM: OTHER_WEEK_NUM,
        SELECTORS: SELECTORS,
        buildActivityKey: buildActivityKey,
        ...
        isSafeUrl: isSafeUrl,
        isValidCachePayload: isValidCachePayload,
