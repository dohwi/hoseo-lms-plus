(function (global, factory) {
    ...
    function findCurrentWeekIndex(sortedWeeks, itemsByWeek, assignsByWeek, activitiesByWeek, now) {
        ...
        var bestPast = -1;
        var bestPastDiff = Infinity;
        for (var index = 0; index < sortedWeeks.length; index += 1) {
            var week = sortedWeeks[index];
            var periodStr = (itemsByWeek.get(week) && itemsByWeek.get(week)[0] ||
                assignsByWeek.get(week) && assignsByWeek.get(week)[0] ||
                activitiesByWeek.get(week) && activitiesByWeek.get(week)[0] || {}).periodStr;
            var range = parsePeriodRange(periodStr, current);
            if (!range) continue;
            if (current >= range.start && current <= range.end) return index;
            var diff = current.getTime() - range.end.getTime();
        ...
        var firstRegularWeek = sortedWeeks.findIndex(function (w) { return w !== OTHER_WEEK_NUM; });
