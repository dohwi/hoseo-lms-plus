(function (global, factory) {
    ...
    function parseNoticePostedDate(row, titleLink) {
        const candidates = Array.from(row.querySelectorAll('time, .date, .posted-date, .time, td, span'))
        ...
        for (const element of candidates) {
            ...
            const match = normalized.match(/(20\d{2})\s*(?:[.\-/년])\s*(\d{1,2})\s*(?:[.\-/월])\s*(\d{1,2})\s*(?:일)?/);
            if (!match) continue;
    ...
    function isNoticePostUrl(url, baseUrl) {
        ...
        try {
            const expectedBase = new URL(baseUrl || core.DEFAULT_BASE_URL);
            const parsed = new URL(url, expectedBase);
            const isReadPath = parsed.pathname === '/mod/ubboard/read.php';
            const isArticleView = parsed.pathname === '/mod/ubboard/view.php' && Boolean(parsed.searchParams.get('articleid'));
    ...
    function parseUbboardNoticePage(html, courseId, courseName, periodMap, baseUrl, now) {
        ...
        rows.forEach(function (row) {
            ...
            const postedDate = parseNoticePostedDate(row, link);
