(function (global, factory) {
    ...
    const MAX_NOTICE_TITLE_LENGTH = 500;
    const MAX_NOTICE_DATE_LENGTH = 100;
    ...
    function parseNoticePostedDate(row, titleLink, now) {
        ...
        for (const element of candidates) {
            const normalized = core.normalizeText(truncate(element.getAttribute('datetime') || element.textContent, MAX_NOTICE_DATE_LENGTH));
    ...
    function getNoticePostUrl(url, baseUrl, boardUrl) {
        if (!url || !core.isSafeUrl(url, baseUrl)) return null;
        try {
            const expectedBase = new URL(baseUrl || core.DEFAULT_BASE_URL);
            const parsed = new URL(url, expectedBase);
            if (parsed.origin !== expectedBase.origin || !parsed.pathname.startsWith('/mod/ubboard/')) return null;

            const board = boardUrl ? new URL(boardUrl, expectedBase) : null;
            const postParams = ['articleid', 'article_id', 'bwid', 'bid', 'postid', 'contentid', 'idx', 'no', 'wr_id'];
            const hasPostId = postParams.some(function (key) { return Boolean(parsed.searchParams.get(key)); });
            const viewModes = ['view', 'read', 'detail', 'article'];
            const hasViewMode = viewModes.includes(String(parsed.searchParams.get('mode') || parsed.searchParams.get('action') || '').toLowerCase());
            const hasInlineAction = parsed.hash && parsed.hash !== '#';
            const isDedicatedReadPath = parsed.pathname === '/mod/ubboard/read.php';
            if (!hasPostId && !hasViewMode && !hasInlineAction && !isDedicatedReadPath) return null;
            if (board && parsed.pathname === board.pathname && parsed.search === board.search && !hasInlineAction) return null;
    ...
    function parseUbboardNoticePage(html, courseId, courseName, periodMap, baseUrl, now, boardUrl) {
        const doc = parseDocument(html);
        const notices = [];
        const seen = new Set();
        const rows = Array.from(doc.querySelectorAll([
            ...
            '.ubboard-list-item', '.board-item', 'article', '.card', '.media'
        ].join(',')));

        rows.slice(0, MAX_NOTICE_CANDIDATES).forEach(function (row) {
            if (notices.length >= MAX_NOTICE_CANDIDATES) return;
            const links = Array.from(row.querySelectorAll('a[href]'));
            const link = links.find(function (anchor) {
                const href = anchor.getAttribute('href') || '';
                return Boolean(core.normalizeText(anchor.textContent) && getNoticePostUrl(href, baseUrl, boardUrl));
            });
            if (!link) return;

            const href = getNoticePostUrl(link.getAttribute('href'), baseUrl, boardUrl);
            ...
            const current = now instanceof Date ? now : new Date();
            const isCurrentAcademicYear = postedDate.year === current.getFullYear() ||
                (current.getMonth() === 0 && postedDate.year === current.getFullYear() - 1 && postedDate.month === 12);
            if (!isCurrentAcademicYear) return;
            ...
            notices.push({
                ...
                type: '공지사항',
                href: href,
                nameHtml: core.sanitizeHtmlToString(doc, truncate(link.innerHTML, MAX_NOTICE_TITLE_LENGTH), { baseUrl: baseUrl }),
    ...
    return {
        ...
        MAX_NOTICE_CANDIDATES: MAX_NOTICE_CANDIDATES
    };
});
