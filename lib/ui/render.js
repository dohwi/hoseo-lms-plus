(function (global, factory) {
    ...
    function createNoticeCard(doc, notices) {
        if (!notices || !notices.length) return null;
        const card = elements.createElement(doc, 'section', {
            className: 'lms-card lms-notice-card',
            attrs: { 'aria-labelledby': 'lms-notice-title' }
        });
        const heading = elements.createElement(doc, 'h4', { className: 'lms-notice-title', attrs: { id: 'lms-notice-title' } });
        heading.appendChild(elements.createSvgIcon(doc, 'lms-notice-icon', null, [
        ...
        heading.appendChild(doc.createTextNode(' 강좌별 공지사항'));
        card.appendChild(heading);

        const list = elements.createElement(doc, 'ul', { className: 'lms-notice-list' });
        notices.forEach(function (notice) {
            const item = elements.createElement(doc, 'li', { className: 'lms-notice-item' });
            item.appendChild(elements.createElement(doc, 'span', { className: 'lms-notice-course', text: notice.courseName }));
            const noticeTitle = core.stripHtml(notice.titleHtml) || '공지사항';
            const link = elements.createElement(doc, 'a', {
                className: 'lms-notice-link',
                attrs: {
                    href: notice.href,
                    target: '_blank',
                    rel: 'noopener noreferrer',
                    'aria-label': notice.courseName + ' ' + noticeTitle + ' (새 창에서 열림)'
                }
            });
            elements.appendSanitizedHtml(link, notice.titleHtml || '공지사항', core.DEFAULT_BASE_URL);
    ...
    function renderDashboard(doc, mount, state) {
        ...
        const noticeCard = createNoticeCard(doc, state.notices || []);
        if (noticeCard) mount.appendChild(noticeCard);
