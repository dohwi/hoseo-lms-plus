(function (global, factory) {
    ...
    function restoreHost(host, mount) {
        if (mount) elements.clearChildren(mount);
    ...
    function createActivityRows(doc, activities, baseUrl, includeWeek) {
        ...
        return sorted.map(function (activity) {
            ...
            let courseContextAdded = false;
            function appendCourseContext(cell) {
                if (!courseContextAdded && !isFirst) {
                    cell.appendChild(elements.createCourseContext(doc, activity.courseName));
                    courseContextAdded = true;
                }
            }

            if (includeWeek) {
                const weekCell = elements.createElement(doc, 'td', { className: 'lms-td-week', text: core.getWeekLabel(activity.weekNum) });
                appendCourseContext(weekCell);
                row.appendChild(weekCell);
            }

            const typeCell = createActivityTypeCell(doc, activity);
            appendCourseContext(typeCell);
            row.appendChild(typeCell);

            const nameCell = elements.createElement(doc, 'td', { className: 'lms-td-left' + (isUrgent ? ' lms-urgent-text' : '') });
            appendCourseContext(nameCell);
            if (isUrgent) {
