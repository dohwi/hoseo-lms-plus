        ...
        function buildActivities(courseId, courseName, sourceActivities, attendanceItems, assignments, quizzes) {
            ...
            sourceActivities.forEach((activity) => {
                ...
                if (!activity.isIgnoredType && isVideoType(activity.type)) {
                    ...
                    const matchedAssign = findMatchedByKey(activity, assignsByWeek.get(activity.weekNum), assignments.concat(quizzes))
                        || findMatchedItem(assignsByWeek.get(activity.weekNum), assignments.concat(quizzes), function (item) {
