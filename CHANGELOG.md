...
- 캐시 스키마·크기 검증, 응답 크기 제한, 로그인 폼 기반 세션 만료 감지
- ZIP allowlist·무결성·재현성 검증과 런타임 의존성 보안 감사 CI

### 보안
- 모든 LMS 링크와 네트워크 요청을 `https://learn.hoseo.ac.kr` 동일 출처로 제한
- 외부 이미지와 링크를 정제 단계에서 차단해 의도하지 않은 외부 요청 방지
- 사용자 식별 실패 시 캐시를 사용하지 않아 공용 환경의 캐시 충돌 방지
- GitHub Actions를 검증된 커밋 SHA로 고정

### 개선
- 과제·퀴즈 상세 요청은 동시 요청 큐로 제어하면서 모든 미완료 항목의 상세 상태를 확인
- 동일 URL 요청 결과를 한 번의 로딩 주기 동안 재사용
- 공지 파싱 후보 및 입력 길이를 제한해 비정상적으로 큰 게시판 처리 비용 제한
- 표기 기준 모달의 배경 비활성화, 정리 수명주기, 스크린 리더용 강좌 문맥 개선
- 크로스 플랫폼 결정적 ZIP 빌드로 동일 소스의 체크섬 재현
